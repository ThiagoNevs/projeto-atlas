...
describe('Finding review decision supersession helpers and DTO', () => {
  ...
  it('uses a deterministic, global and case-sensitive operation fingerprint', () => {
    expect(decisionSupersessionRequestFingerprint('supersede-ABC')).toBe(
      decisionSupersessionRequestFingerprint('supersede-ABC'),
    );
    expect(decisionSupersessionRequestFingerprint('supersede-ABC')).not.toBe(
      decisionSupersessionRequestFingerprint('supersede-abc'),
    );
    expect(decisionSupersessionRequestFingerprint('supersede-ABC')).not.toBe(
      decisionRequestFingerprint('supersede-ABC'),
    );
  });
});
...
describe('POST /conflict-review-cases/:caseId/decisions/:decisionId/supersessions', () => {
  ...
  it.each([
    ...
    [FindingReviewIdentityConclusion.DIFFERENT_ASSETS, FindingReviewIdentityConclusion.DIFFERENT_ASSETS],
    ...
    const justification = to === from ? 'Justificativa materialmente corrigida.' : 'Nova conclusão.';
    const response = await postSupersession(
      fixture,
      `success-${from}-${to}-${randomUUID()}`,
      validPayload(fixture.version, to, `  ${justification}\nLinha preservada.  `, '  Erro humano.  '),
    );
    ...
    expect(await prisma.findingReviewDecision.findUniqueOrThrow({
      where: { id: fixture.decisionId! },
    })).toEqual(original);
  ...
  it('persists canonical event and AuditLog and exposes safe history without raw key', async () => {
    ...
    const body = responseBody<SupersessionBody>(response);
    const fingerprint = decisionSupersessionRequestFingerprint(rawKey);
    expect(response.status).toBe(201);
    ...
    expect(detailBody.events.find(
      (item) => item.eventType === FINDING_REVIEW_DECISION_SUPERSEDED_EVENT,
  ...
  it('rejects an effective no-op even when correctionReason changes', async () => {
    ...
    const response = await postSupersession(
      fixture,
      `no-op-${randomUUID()}`,
      validPayload(2, FindingReviewIdentityConclusion.SAME_ASSET, '  Conteúdo canônico.  ', 'Outro motivo.'),
    );
    expect(response.status).toBe(422);
  ...
  it('returns the public structural and lookup errors without writes', async () => {
    ...
    const extraField = await postSupersession(
      fixture,
      `extra-${randomUUID()}`,
  ...
  it.each([
    ...
    const fingerprint = decisionSupersessionRequestFingerprint(key);
    const event = await prisma.findingReviewEvent.findUniqueOrThrow({
      where: { caseId_requestId: { caseId: fixture.caseId, requestId: fingerprint } },
    });
  ...
  it.each([
    ...
    if (dimension === 'conclusion') payload.identityConclusion = FindingReviewIdentityConclusion.SAME_ASSET;
  ...
  it('serializes concurrent supersession and resolution deterministically by CAS', async () => {
    ...
    const resolution = request(server)
    ...
    const [supersessionResponse, resolutionResponse] = await Promise.all([supersession, resolution]);
  ...
  it.each(['decision', 'event', 'audit'] as const)(
    ...
    async (step) => {
      ...
      expect(await effects(fixture.caseId)).toEqual(effectsBefore);
      expect(await inventoryCounts()).toEqual(inventoryBefore);
      expect(await prisma.findingReviewDecision.findUnique({
        where: { requestFingerprint: decisionSupersessionRequestFingerprint(key) },
      })).toBeNull();
  ...
  it('rejects direct supersession after resolution and allows it after explicit reopen', async () => {
    ...
    await postSupersession(
      fixture,
      `resolved-direct-${randomUUID()}`,
      validPayload(3),
    ).expect(422);
